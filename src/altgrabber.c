...
alt_grabber_event_filter (GdkXEvent *xevent,
                          GdkEvent  *event,
                          gpointer   user_data)
{
...
static gboolean
alt_grabber_finish_clear (gpointer data)
{
...
static void
alt_grabber_finalize (GObject *object)
{
  ...
  G_OBJECT_CLASS (alt_grabber_parent_class)
    ->finalize (object);
}

static void
alt_grabber_init (AltGrabber *grabber)
{
  grabber->grabs = g_hash_table_new (NULL, NULL);
}

static void
alt_grabber_class_init (AltGrabberClass *class)
{
  class->finalize = alt_grabber_finalize;
}

static AltGrabber *
alt_grabber_new (GdkScreen *screen)
{
  AltGrabber *grabber;
  GdkDisplay *display;
 ...
 **/
AltGrabber *
alt_grabber_get_for_screen (GdkScreen *screen)
{
  AltGrabber *grabber;
  GSList *node;
 ...
 **/
void
alt_grabber_clear (AltGrabber *grabber)
{
      ...
      for (i = 0; i < n_codes; i++)
        {
          ...
          if (!g_hash_table_lookup_extended (grabber->grabs, GINT_TO_POINTER (codes[i].keycode), NULL, &list))
            {
              guint mask;

              for (mask = 0; mask < 0x100; mask++)
                if ((mask & (ShiftMask | ControlMask | Mod1Mask)) == Mod1Mask)
                  XGrabKey (dpy, codes[i].keycode, mask, root, False, GrabModeAsync, GrabModeAsync);
            }

          list = g_slist_prepend (list, grab);
          g_hash_table_insert (grabber->grabs, GINT_TO_POINTER (codes[i].keycode), list);
        }
